	...
	PF_OutData		*out_data)
{
	...
	switch (in_data->appl_id){
		...
		case 'FXTC':
			...
			{
				if (in_data->version.major == PF_AE140_PLUG_IN_VERSION && in_data->version.minor >= PF_AE140_PLUG_IN_SUBVERS) {
					PF_SPRINTF(out_data->return_msg, "Running in After Effects CC 2017 (14.0) or later.");
